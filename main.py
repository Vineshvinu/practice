...
@TelegramClient.on_message(filters.command("about"))
async def about_cmd(client, message):
    await message.reply_text("Bot status")

@TelegramClient.on_message(filters.command("start"))
...
@TelegramClient.on_message(filters.command("help"))
async def help_cmd(client, message):
    await message.reply_text("```I think you are genius and you can't get any help from others```")

@TelegramClient.on_message(filters.command("love"))
async def love_cmd(client, message):
    await message.reply_text("```ME 2 ❤️ you```")

@TelegramClient.on_message(filters.command("code"))
